...
class PlayerRepository:
    ...
    async def get_by_username(self, username: str) -> Optional[Player]:
        stmt = select(Player).where(Player.username == username).limit(1)
        return await self.session.scalar(stmt)

    async def get_by_workspace(self, workspace: Workspace, filter: Optional[str] = None) -> list[Player]:
        stmt = select(Player).join(Player.creator).where(WorkspaceProfile.workspace_id == workspace.id)
        if filter is not None:
            stmt = stmt.filter(Player.username.ilike(filter))
        return list((await self.session.scalars(stmt)).all())
    
    async def create(self, creator: WorkspaceProfile, username: str) -> Player:
    ...
    async def get_or_create_roles(self, player: Player, workspace_profile: WorkspaceProfile) -> PlayerRoles:
        get_stmt = select(PlayerRoles).where(PlayerRoles.creator_id == workspace_profile.id, PlayerRoles.player_id == player.id).limit(1)
        player_roles = await self.session.scalar(get_stmt)
        if player_roles is not None:
            return player_roles
        new_player_roles = PlayerRoles(creator_id=workspace_profile.id, player_id=player.id)
