...
def generate_responses_for_endpoint(allowed_exceptions: list[type[Exception]]):
    responses = {}
    grouped_by_status = {}
    ...
    for status_code, descriptions in grouped_by_status.items():
        if len(descriptions) == 1:
            responses[status_code] = {"description": descriptions[0], "content": {
                "application/json": {
                    "example": {"detail": descriptions[0]}
                }
            }, }
        else:
